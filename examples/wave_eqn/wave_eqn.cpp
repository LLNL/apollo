...
int main(int RAJA_UNUSED_ARG(argc), char **RAJA_UNUSED_ARG(argv[]))
{
  ...
  printf("Threads: %d, schedule: %d, chunk: %d\n", threads, schedule, chunksize);
#endif
...
#ifdef USE_TAU
  TAU_PROFILE_TIMER(timer, "main loop", "", TAU_DEFAULT);
  TAU_PROFILE_START(timer);
#endif
  for (int k = 0; k < nt; ++k) {

#ifdef USE_TAU
    //TAU_PROFILE_TIMER(timer, "parallel region", "", TAU_DEFAULT);
    //TAU_PROFILE_START(timer);
#endif
  ...
  TAU_PROFILE_STOP(timer);
#endif
  printf("Ran for %f iterations\n", nt);
  printf("Published %f times\n", nt/1000.0);
...
{
#ifdef USE_TAU
  //TAU_PROFILE_TIMER(timer, __func__, "", TAU_DEFAULT);
  //TAU_PROFILE_START(timer);
#endif
