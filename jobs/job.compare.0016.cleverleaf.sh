...
# 64 cleverleaf apps
# 1 controller
# 16 "misc just in case"
#
#SBATCH --job-name="APOLLO:COMPARE.64@16.cleverleaf.test"
#SBATCH -N 17
#SBATCH -n 81
#SBATCH -t 180
#
export EXPERIMENT_JOB_TITLE="COMPARE.0016.cleverleaf"  # <-- creates output path!
#
export APPLICATION_RANKS="64"         # ^__ make sure to change SBATCH node counts!
export SOS_AGGREGATOR_COUNT="1"       # <-- actual aggregator count
export EXPERIMENT_NODE_COUNT="16"     # <-- is SBATCH -N count, incl/extra agg. node
#
###################################################################################
#
...
#
    ...
    echo ">>>> OK."
    echo ""
    echo "Running default Cleverleaf ${APPLICATION_RANKS} ranks on ${EXPERIMENT_NODE_COUNT} nodes..."
    echo ""
