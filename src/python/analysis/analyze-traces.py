...
def main():
    ...
    for i in range(args.nranks):
        ...
        if args.rr:
            accuracy['RoundRobin'] =\
                compute_accuracy(
                    i, data_map['RoundRobin'], opt, args.nstatic+1, DIFF_PCT, 'RoundRobin', 'DecisionTree')
        if args.random:
            accuracy['Random'] =\
                compute_accuracy(
                    i, data_map['Random'], opt, args.nstatic+1, DIFF_PCT, 'Random', 'DecisionTree')
        if args.load:
            accuracy['Load'] =\
                compute_accuracy(
                    i, data_map['Load'], opt, args.nstatic+1, DIFF_PCT, 'Load', 'Load')
        if args.opt:
            accuracy['Optimal'] =\
                compute_accuracy(
                    i, data_map['Optimal'], opt, args.nstatic+1, DIFF_PCT, 'Optimal', 'Optimal')
