...
def compute_speedup(xtime_per_pol, model, min_xtime_static_pol):
    print('%s speedup vs defaults (Static,0 / %s): %.3f' %
          (
              model,
              model,
    ...
    print('%s speedup vs best static (%s / %s): %.3f' %
          (
              model,
...
def compute_accuracy(rank, data, opt_data, npolicies, DIFF_PCT, init_model, trained_model):
    ...
    print('Rank %d %s|%s accuracy %.2f%%' %
          (rank, init_model, trained_model, (nmatch*100.0)/nrows))
    # input('cont')

    (hist, bins) = np.histogram(
        opt_region_idx['policy'], bins=np.arange(npolicies))
    print('Rank %d Opt' % (rank), *list(zip(bins[:-1], hist)))
    (hist, bins) = np.histogram(
        data_region_idx['policy'], bins=np.arange(npolicies))
    print('Rank %d %s|%s' % (rank, init_model, trained_model),
          *list(zip(bins[:-1], hist)))
    ...
    opt_region_idx = opt_region_idx[~opt_region_idx.index.isin(
        cond[cond['policy'] == True].index)]

    if int(nmatch) < nrows:
        diff_opt_rr = abs(
            opt_region_idx['xtime'] - data_region_idx['xtime'])*100.0 / opt_region_idx['xtime']
        try:
...
def main():
    parser = argparse.ArgumentParser(
        description='Analyze exhaustive experiments for a program.')
    parser.add_argument('--nstatic', type=int,
                        help='the number of policies to test.', required=True)
    parser.add_argument('--nranks', type=int,
                        help='the number of processes (ranks).', required=True)
    parser.add_argument(
    ...
    parser.add_argument('--random', help='include Random training traces.',
                        action='store_true', default=False)
    parser.add_argument('--load', help='include Load model.',
                        action='store_true', default=False)
    parser.add_argument('--opt', help='include optimal oracle model.',
                        action='store_true', default=False)
    parser.add_argument('--write-opt', help='write optimal policy csv.',
                        action='store_true', default=False)
    ...
    if args.rr:
        ...
        data_map['RoundRobin'] = pd.concat(
            csv_map.values(), ignore_index=True, sort=True)

    if args.random:
        csv_map = {}
        read_csv(args.dir, 'Random', csv_map)
        data_map['Random'] = pd.concat(
            csv_map.values(), ignore_index=True, sort=True)

    if args.load:
        csv_map = {}
        read_csv(args.dir, 'Load', csv_map)
        data_map['Load'] = pd.concat(
            csv_map.values(), ignore_index=True, sort=True)

    if args.opt:
        csv_map = {}
        read_csv(args.dir, 'Optimal', csv_map)
        data_map['Optimal'] = pd.concat(
            csv_map.values(), ignore_index=True, sort=True)

    print()
    # Compute execution times per rank and per policy.
    for rank in range(args.nranks):
        xtime_per_pol = {}
        for j in range(args.nstatic):
            compute_execution_time(
                rank, data_map['Static'], xtime_per_pol, 'Static,%d' % (j))
        if args.rr:
            compute_execution_time(
                rank, data_map['RoundRobin'], xtime_per_pol, 'RoundRobin', 'DecisionTree')
        if args.random:
            compute_execution_time(
                rank, data_map['Random'], xtime_per_pol, 'Random', 'DecisionTree')
        if args.load:
            compute_execution_time(
                rank, data_map['Load'], xtime_per_pol, 'Load')
        if args.opt:
            compute_execution_time(
                rank, data_map['Optimal'], xtime_per_pol, 'Optimal')

        print('=== Computing optimal policy selection...')
        t1 = time.perf_counter()
        # Find optimal policy selection (fastest way): select by rank and training equals to Static;
        # sort by xtime;
        ...
        opt = data_map['Static'].loc[(data_map['Static']['rankid'] == rank)]\
        ...
        #    print('=========== OPT =============\n', opt.to_string(), '\n=============================',
        #          file=f)
        # NOTE: This assertion does not hold if regions do not have the same number of policies.
        # assert len(opt.index) == len(
        #    data_map['Static'].index)/args.nstatic, 'Data error'
        ...
        if args.rr:
            accuracy['RoundRobin'] =\
                compute_accuracy(
                    rank, data_map['RoundRobin'], opt, args.nstatic+1, DIFF_PCT, 'RoundRobin', 'DecisionTree')
        if args.random:
            accuracy['Random'] =\
                compute_accuracy(
                    rank, data_map['Random'], opt, args.nstatic+1, DIFF_PCT, 'Random', 'DecisionTree')
        if args.load:
            accuracy['Load'] =\
                compute_accuracy(
                    rank, data_map['Load'], opt, args.nstatic+1, DIFF_PCT, 'Load', 'Load')
        if args.opt:
            accuracy['Optimal'] =\
                compute_accuracy(
                    rank, data_map['Optimal'], opt, args.nstatic+1, DIFF_PCT, 'Optimal', 'Optimal')
